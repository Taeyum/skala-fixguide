...
/**
 * AI 검증 3종 Mock (가이드: "AI 서비스는 Mock API로 JSON 반환").
...
public class MockAgentEngine {
    ...
    /** ERD 6장 payload_json 구조: A1·A2는 items[], A3는 documents[] */
    ...
    private String specSummary(WorkRequest wr) {
        if (wr.getSpecJson() == null || wr.getSpecJson().isEmpty()) {
            return "스펙 미입력";
        }
        return wr.getSpecJson().entrySet().stream()
    ...
    private String operatingSummary(WorkRequest wr) {
        if (wr.getOperatingCondition() == null || wr.getOperatingCondition().isEmpty()) {
            return "미입력";
        }
        return wr.getOperatingCondition().entrySet().stream()
                .map(e -> e.getKey() + " " + e.getValue())
                .reduce((a, b) -> a + ", " + b)
                .orElse("미입력");
    }
